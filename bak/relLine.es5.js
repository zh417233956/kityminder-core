...
define(function(require, exports, module) {
    ...
    var PlainDraggable = require('../connect/plain-draggable').PlainDraggable;
    ...
    Module.register('relLine', function() {
        ...
        function createRelLine(startNode, endNode) {
            ...
            var endElement = document.getElementById(endElementId); //获取指定节点的Element
            ...
            line.start = LeaderLine.pointAnchor(startElement, {
                x: startElement.getBBox().width / 2.0,
                y: 0
            });
        ...
        function createCircleFun(line) {
            ...
            var lineData = km._relLine[km._relLine_obj.indexOf(line)]; // var svg = km.getRenderContainer().container.node;
        ...
        function updateCirclePosition(line) {
            ...
            if (_p1 && lineData.ctrl) {
                ...
                _line2.y1.baseVal.value = _p2.getBoundingClientRect().y; // line.setOptions({
        ...
        var RelLineCommand = kity.createClass('RelLineCommand', {
            ...
            execute: function execute(km) {
                ...
                if (curNode) {
                    //关系线指向终点函数
                    var relLineMouseFun = function relLineMouseFun() {
                        ...
                        if (_curNode) {
                            try {
                                ...
                                if (newLine.start != __endNode) {
                                    newLine.end = __endNode; //终点定点到中间点
                                    ...
                                    km.getRenderContainer().container.removeEventListener("mousemove", relLineMouseMVFun); //记录关系线数据

                                    _relLine.endId = _curNode.data.id || "";

                                    km._relLine.push(_relLine); //打点
                    ...
                    var relLineMouseMVFun = function relLineMouseMVFun() {
                    ...
                    var startElement = document.getElementById(startElementId);
                    var newLine = new LeaderLine(startElement, endNodeAreaAnchor, relLineOption); //起点定点到中间点                   
                    ...
                    km._relLine_obj.push(newLine); //挂载拖拽事件
                    ...
                    km.getRenderContainer().container.addEventListener("mousedown", relLineMouseFun); //鼠标在画布上移动，关系线跟随效果
        ...
        var LeaderLineRenderer = kity.createClass('LeaderLineRenderer', {
            ...
            constructor: function constructor(node) {
                this.callBase();
                this.create(); // this.setId(utils.uuid('relLine'));
            },
            create: function create() {
                try {
                    setTimeout(function() {
                        ...
                        km._relLine_obj = km._relLine_obj || []; //挂载渲染关系线事件
                        ...
                        km._relLine_render = function(time, expand) {
                            ...
                            setTimeout(function() {
                                ...
                                if (km._relLine_obj) {
                                    var curNode = km.getSelectedNode();
                                    var curNodeChild = curNode != undefined ? curNode.children : null; // 折叠/展开，需要隐藏/显示关系线

                                    var expand_relLine_fun = function expand_relLine_fun(nodeChilds, itemLine, lineData) {
                                        //折叠/展开，需要隐藏/显示关系先
                        ...
                        relLineArray.forEach(function(_lineItem, _lineIndex) {
                            ...
                            createRelLine(km.getNodeById(_lineItem.startId), km.getNodeById(_lineItem.endId)); // [startElementId, endElementId].forEach((elementId) => {
                            //     //拖拽Node并移动
